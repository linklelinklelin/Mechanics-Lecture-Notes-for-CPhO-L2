...
#let ct-fonts = (
  ...
  main: ("TeX Gyre Pagella", "Libertinus Serif", "New Computer Modern","Noto Serif CJK SC"),
...
// Page setup
  set page(
    ...
    header: context {
      ...
      let has-chapter-on-page = query(heading.where(level: 1))
        .any(it => it.location().page() == physical-page)
      let has-part-on-page = query(selector(<ct-part-marker>))
        .any(it => it.location().page() == physical-page)
